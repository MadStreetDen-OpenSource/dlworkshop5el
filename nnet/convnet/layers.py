...
class Conv(Layer, ParamMixin):
    ...
    def __init__(self, n_feats, filter_shape, strides, weight_scale,
        ...
        """
        Init conv layer
        :param n_feats: Number of output feature maps
        :param filter_shape: Size of filter - tuple of length 2
        :param strides: Convolution stride  # currently implemented with stride=1 only.
        :param weight_scale: Random weight initialization scale
        :param weight_decay: Weight decay factor
        :param padding_mode: same, full, or ''
        :param border_mode:
        :return:
        """
...
class Pool(Layer):
    ...
    def __init__(self, pool_shape=(3, 3), strides=(1, 1), mode='max'):
        """
        Init a Pool Layer
        :param pool_shape: size of pooling kernel
        :param strides: pooling stride
        :param mode: 'max' - max pooling  # only max pooling supported
        :return:
        """
        self.mode = mode
    ...
    def fprop(self, input):
        ...
        # Calling Cython code for fprop, see conv.pyx for code
...
class Flatten(Layer):
    """
    Flattens a conv layer in order to make it compatible as input to a Linear layer.
    """
    def fprop(self, input):
        self.last_input_shape = input.shape
